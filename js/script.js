...
const timetText = document.querySelector(".time b");
const inputField = document.querySelector("input");
const refreshBtn = document.querySelector(".refersh-word");
...
const initTimer = maxTime => {
    timer = setInterval(() => {
        if(maxTime > 0) {
            maxTime--;
            return timetText.innerText = maxTime;
        }
        clearInterval(timer);
...
const initGame = () => {
 ...
 for(let i=wordArray.length-1; i>0 ;i--){
    let j = Math.floor(Math.random() * (i+1));  // getting random number
     //shuffling and swipping wordaArray letters randomly
